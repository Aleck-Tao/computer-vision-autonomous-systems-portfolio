...
| [Safe neural control certificates](https://github.com/Aleck-Tao/safe-neural-control-certificates) | Can an arbitrary finite neural action be projected into an analytically feasible robust interval for a declared uncertain plant? | Independent certificate recomputation, five-controller comparison, explicit negative controls, 90 closed-loop runs, 36,012 implementation checks, v0.1.0 and six-job CI | Exact only for the scalar sampled-data model and assumptions; no MIMO, continuous-time, physical-system, HIL, or intrinsic-neural-stability claim |
